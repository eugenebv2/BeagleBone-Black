...
void EEPROM_erase(int f){
	...
	unsigned adr = 0;
	    ...
	    unsigned int cmd[EEPROM_ADDRESS_SIZE] = {adr >> 8, (unsigned int )adr};
	    printf("page %i erase address: 0x%x, 0x%x \n",page,cmd[0],cmd[1]);
		
	    write(f, cmd, 2);
       	    write(f, zeroArray, EEPROM_PAGE_SIZE);
        }
}
void EEPROM_erase_page(int f, unsigned int page){
	unsigned adr = 0;
	adr = page * EEPROM_PAGE_SIZE * 8 ;
   	unsigned int zeroArray[EEPROM_PAGE_SIZE] = {0};
	unsigned int cmd[EEPROM_ADDRESS_SIZE] = {adr >> 8, (unsigned int )adr};
